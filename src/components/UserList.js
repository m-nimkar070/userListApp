...
const UserList = () => {
  ...
  const postCall=async()=>{
    await axios.post(`https://jsonplaceholder.typicode.com/users/`,{id:users[users.length-1].id+1,name:name,email:email,phone:phone})
  ...
  const handleSubmit = (e) => {
    ...
    postCall();
    setUsers((prev)=>[...prev,{
      id:users[users.length-1].id+1,name:name,email:email,phone:phone
    }])
    setName("");
    setEmail("");
    setPhone("");
  ...
  const handleUpdate = (id)=>{
    ...
    axios.put(`https://jsonplaceholder.typicode.com/users/${editId}`, {id:editId,name:fullname,email:updateEmail,phone:updatePhone})
